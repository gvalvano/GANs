...
class MLP(object):
    ...
    def build(self):
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            input_layer = tf.compat.v1.layers.batch_normalization(input_layer, training=self.is_training)
            ...
            hidden_layer = tf.compat.v1.layers.batch_normalization(hidden_layer, training=self.is_training)
            hidden_layer = tf.nn.relu(hidden_layer)

            output_layer = layers.dense(hidden_layer, self.n_out, bias_initializer=b_init)
            output_layer = tf.compat.v1.layers.batch_normalization(output_layer, training=self.is_training)
