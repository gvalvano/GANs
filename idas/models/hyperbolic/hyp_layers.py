...
def hyp_pixel_embedding(incoming, radius=1.0):
    ...
    out_shape = tf.convert_to_tensor(value=[-1, W, H, C])  # [batch_size, W, H, num_classes])
...
def hyp_dense(incoming, shape, activation='id', radius=1.0, bias_geom='hyp', mlr_geom='hyp', dropout=1.0):
    ...
    with tf.compat.v1.variable_scope('HyperbolicDenseLayer'):

        hidden_dim, before_mlr_dim = shape

        # Define variables for the feed-forward layer: W * x + b
        W = tf.compat.v1.get_variable('W_hyp', dtype=tf.float32, shape=[hidden_dim, before_mlr_dim],
                            initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
        b = tf.compat.v1.get_variable('b_hyp', dtype=tf.float32, shape=[1, before_mlr_dim],
...
def hyp_mlr(incoming, before_mlr_dim, num_classes, radius=1.0, reuse=tf.compat.v1.AUTO_REUSE,
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        for cl in range(num_classes):
            A_mlr.append(tf.compat.v1.get_variable('A_mlr' + str(cl),
                                         dtype=tf.float32,
                                         shape=[1, before_mlr_dim],
                                         initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")))

            P_mlr.append(tf.compat.v1.get_variable('P_mlr' + str(cl),
                                         dtype=tf.float32,
                                         shape=[1, before_mlr_dim],
                                         initializer=tf.compat.v1.constant_initializer(0.0)))
