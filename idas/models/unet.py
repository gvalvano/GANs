...
class UNet(object):
    ...
    def build(self, incoming, reuse=tf.compat.v1.AUTO_REUSE):
        """
        Build the model.
        """
    ...
    def _encode_brick(incoming, nb_filters, is_training, scope, use_bn=True, trainable=True):
        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            with tf.compat.v1.variable_scope('concat_layer_out'):
    ...
    def _decode_brick(self, incoming, concat_layer_in, nb_filters, is_training, scope, use_bn=True):
        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            if self.upsampling_mode == 'deconv':
                ...
                upsampled = tf.image.resize(incoming, size=[new_height, new_width], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    ...
    def get_prediction(self, one_hot=False):
        if one_hot:
            return tf.one_hot(tf.argmax(input=self.prediction, axis=-1), self.n_out)
