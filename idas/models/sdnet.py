...
he_init = tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0, mode=('FAN_IN').lower(), distribution=("uniform" if False else "truncated_normal"))
...
class SDNet(object):
    ...
    def build(self, input_image, reuse=tf.compat.v1.AUTO_REUSE):
        """
        Build the model.
        """
        with tf.compat.v1.variable_scope(self.name, reuse=reuse):
            ...
            if self.use_segmentor:
                ...
                non_hart = tf.reduce_sum(input_tensor=non_heart_channels, axis=-1, keepdims=True)
    ...
    def build_anatomy_encoder(self, input_image):
        with tf.compat.v1.variable_scope('AnatomyEncoder'):
        ...
        with tf.compat.v1.variable_scope('RoundingLayer'):
    ...
    def build_modality_encoder(self, input_image, hard_anatomy):
        with tf.compat.v1.variable_scope('ModalityEncoder'):
    ...
    def build_segmentor(self, hard_anatomy, n_classes):
        with tf.compat.v1.variable_scope('Segmentor'):
    ...
    def get_pred_mask(self, one_hot, output=None):
        ...
        if not one_hot:
            ...
            return tf.one_hot(tf.argmax(input=self.pred_mask, axis=-1), self.n_classes)
