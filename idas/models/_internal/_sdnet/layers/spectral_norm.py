...
class SpectralNormConv2D(tf.keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        with tf.compat.v1.variable_scope(self.scope):
            w = tf.compat.v1.get_variable("kernel", shape=[self.kernel_size, self.kernel_size, inputs.get_shape()[-1], self.filters])
            b = tf.compat.v1.get_variable("bias", [self.filters], initializer=tf.compat.v1.constant_initializer(0.0))

            conv_norm = tf.nn.conv2d(input=inputs, filters=spectral_norm(w, self.iteration),
