...
def partition_spline_curve(alpha):
  ...
  """
    ...
    assert_ops = [tf.Assert(tf.reduce_all(input_tensor=alpha >= 0.), [alpha])]
    with tf.control_dependencies(assert_ops):
        x = tf.compat.v1.where(alpha < 4, (c(2.25) * alpha - c(4.5)) /
...
def inv_partition_spline_curve(x):
    ...
    assert_ops = [tf.Assert(tf.reduce_all(input_tensor=x >= 0.), [x])]
    with tf.control_dependencies(assert_ops):
        alpha = tf.compat.v1.where(
            x < 8,
            c(0.5) * x + tf.compat.v1.where(
                x <= 4,
...
def nllfun(x, alpha, scale):
  ...
  """
    ...
    tf.compat.v1.assert_type(scale, x.dtype)
    tf.compat.v1.assert_type(alpha, x.dtype)
    assert_ops = [
        # `scale` must be > 0.
        tf.Assert(tf.reduce_all(input_tensor=scale > 0.), [scale]),
        # `alpha` must be >= 0.
        tf.Assert(tf.reduce_all(input_tensor=alpha >= 0.), [alpha]),
    ]
    with tf.control_dependencies(assert_ops):
