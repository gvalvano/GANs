  ...
  """
    ...
    with tf.control_dependencies(assert_ops):
        ...
        if direction == 0:
            ...
            f_ex = f[tf.newaxis, :]
            strides = [1, 1, 2, 1]
        y = tf.nn.conv2d(input=x_padded[:, :, :, tf.newaxis],
                         filters=tf.cast(f_ex, x.dtype)[:, :, tf.newaxis, tf.newaxis],
                         strides=strides, padding='VALID')[:, :, :, 0]
        return y
...
def _upsample(x, up_sz, f, direction, shift):
  ...
  """
    ...
    with tf.control_dependencies(assert_ops):
        ...
        if direction == 0:
            sz_ex = tf.shape(input=x) * [1, 2, 1]
        elif direction == 1:
            sz_ex = tf.shape(input=x) * [1, 1, 2]
        if shift == 0:
        ...
        x_undecimated = tf.pad(tensor=x_undecimated,
                               paddings=[[0, 0], [0, up_sz[0] - tf.shape(input=x_undecimated)[1]],
                                [0, up_sz[1] - tf.shape(input=x_undecimated)[2]]])
...
def construct(im, num_levels, wavelet_type):
  ...
  """
    ...
    max_num_levels = get_max_num_levels(tf.shape(input=im))
    assert_ops = [
        tf.Assert(
            tf.greater_equal(max_num_levels, num_levels),
            [tf.shape(input=im), num_levels, max_num_levels])
    ]
    with tf.control_dependencies(assert_ops):
...
def collapse(pyr, wavelet_type):
  ...
  """
    ...
    for d in range(num_levels - 1, -1, -1):
        ...
        up_sz = (tf.shape(input=hi_lo)[1] + tf.shape(input=lo_hi)[1],
                 tf.shape(input=lo_hi)[2] + tf.shape(input=hi_lo)[2])
        lo_sz = (tf.shape(input=im)[1], up_sz[1])
        hi_sz = (tf.shape(input=hi_hi)[1], up_sz[1])
        im = (
                _upsample(
