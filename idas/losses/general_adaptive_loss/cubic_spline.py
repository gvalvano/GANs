...
def interpolate1d(x, values, tangents):
  ...
  tf.compat.v1.assert_type(values, x.dtype)
  tf.compat.v1.assert_type(tangents, x.dtype)
  float_dtype = x.dtype
  assert_ops = [
      # `values` must be a vector.
      tf.Assert(tf.equal(tf.rank(values), 1), [tf.shape(input=values)]),
      # `tangents` must be a vector.
      tf.Assert(tf.equal(tf.rank(tangents), 1), [tf.shape(input=values)]),
      # `values` and `tangents` must have the same length.
      tf.Assert(
          tf.equal(tf.shape(input=values)[0],
                   tf.shape(input=tangents)[0]),
          [tf.shape(input=values)[0], tf.shape(input=tangents)[0]]),
  ]
  with tf.control_dependencies(assert_ops):
    ...
    x_lo = tf.cast(
        tf.floor(
            tf.clip_by_value(x, 0., tf.cast(
                tf.shape(input=values)[0] - 2, float_dtype))), tf.int32)
    x_hi = x_lo + 1
    ...
    return tf.compat.v1.where(t < 0., value_before,
                    tf.compat.v1.where(t > 1., value_after, value_mid))
