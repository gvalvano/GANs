...
def inv_softplus(y):
    ...
    return tf.compat.v1.where(y > 87.5, y, tf.math.log(tf.math.expm1(y)))
...
def image_dct(image):
    ...
    dct_y = tf.transpose(a=tf.signal.dct(image, type=2, norm='ortho'), perm=[0, 2, 1])
    dct_x = tf.transpose(a=tf.signal.dct(dct_y, type=2, norm='ortho'), perm=[0, 2, 1])
    return dct_x


def image_idct(dct_x):
    """Inverts image_dct(), by performing a type-III DCT."""
    dct_y = tf.signal.idct(tf.transpose(a=dct_x, perm=[0, 2, 1]), type=2, norm='ortho')
    image = tf.signal.idct(tf.transpose(a=dct_y, perm=[0, 2, 1]), type=2, norm='ortho')
    return image


def compute_jacobian(f, x):
    """Computes the Jacobian of function `f` with respect to input `x`."""
    x_ph = tf.compat.v1.placeholder(tf.float32, x.shape)
    vec = lambda x: tf.reshape(x, [-1])
    jacobian = tf.stack(
        [vec(tf.gradients(ys=vec(f(x_ph))[d], xs=x_ph)[0]) for d in range(x.size)], 1)
